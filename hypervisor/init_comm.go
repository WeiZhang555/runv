...
func waitConsoleOutput(ctx *VmContext) {
	...
	if err != nil {
		logrus.Error("failed to connected to ", ctx.ConsoleSockName, " ", err.Error())
		return
	}
...
func waitInitReady(ctx *VmContext) {
	...
	if err != nil {
		logrus.Error("Cannot connect to hyper socket ", err.Error())
	...
	if err != nil {
		logrus.Error("read init message failed... ", err.Error())
		ctx.Hub <- &InitFailedEvent{
		...
		logrus.Warningf("Get init message %d", msg.Code)
		ctx.Hub <- &InitFailedEvent{
...
func connectToInit(ctx *VmContext) {
	conn, err := utils.UnixSocketConnect(ctx.HyperSockName)
	if err != nil {
		logrus.Error("Cannot re-connect to hyper socket ", err.Error())
		ctx.Hub <- &InitFailedEvent{
	...
	for looping {
		...
		if cmd.Code == INIT_ACK || cmd.Code == INIT_ERROR {
			if len(cmds) > 0 {
				...
				logrus.Error("got ack but no command in queue")
			}
