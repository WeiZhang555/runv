...
func (qmp *QmpResponse) UnmarshalJSON(raw []byte) error {
	...
	logrus.Info("[RUNV] got a message ", string(raw))
	if _, ok := tmp["event"]; ok {
		msg := &QmpEvent{}
		err = json.Unmarshal(raw, msg)
		logrus.Info("[RUNV] got event: ", msg.Type)
		qmp.msg = msg
...
func qmpReceiver(qmp chan QmpInteraction, wait chan int, decoder *json.Decoder) {
	logrus.Info("[RUNV] Begin receive QMP message")
	for {
		rsp := &QmpResponse{}
		if err := decoder.Decode(rsp); err != nil {
			logrus.Info("[RUNV] QMP exit as got error: ", err.Error())
		...
		if msg.MessageType() == QMP_EVENT && msg.(*QmpEvent).Type == QMP_EVENT_SHUTDOWN {
			logrus.Info("[RUNV] Shutdown, quit QMP receiver")
			return
		}
	}
}
...
func qmpInitializer(ctx *hypervisor.VmContext) {
	...
	logrus.Info("[RUNV] connected to ", qc.qmpSockName)
	...
	logrus.Info("[RUNV] begin qmp init...")
	...
	logrus.Info("[RUNV] got qmp welcome, now sending command qmp_capabilities")
	...
	logrus.Info("[RUNV] waiting for response")
	rsp := &QmpResponse{}
	...
	logrus.Info("[RUNV] got for response")

	if rsp.msg.MessageType() == QMP_RESULT {
		logrus.Info("[RUNV] QMP connection initialized")
		qc.qmp <- &QmpInit{
...
func qmpCommander(handler chan QmpInteraction, conn *net.UnixConn, session *QmpSession, feedback chan QmpInteraction) {
	logrus.Info("[RUNV] Begin process command session")
	for _, cmd := range session.commands {
		...
		for repeat := 0; !success && repeat < 3; repeat++ {
			...
			if len(cmd.Scm) > 0 {
				logrus.Infof("[RUNV] send cmd with scm (%d bytes) (%d) %s", len(cmd.Scm), repeat+1, string(msg))
				f, _ := conn.File()
				fd := f.Fd()
				syscall.Sendmsg(int(fd), msg, cmd.Scm, nil, 0)
			} else {
				logrus.Infof("[RUNV] sending command (%d) %s", repeat+1, string(msg))
				conn.Write(msg)
			}

			res, ok := <-feedback
			if !ok {
				logrus.Info("[RUNV] QMP command result chan closed")
				return
			}
			...
			case QMP_INTERNAL_ERROR:
				logrus.Info("[RUNV] QMP quit... commander quit... ")
				return
			}
		}
...
func qmpHandler(ctx *hypervisor.VmContext) {
	...
	loop := func(msg QmpInteraction) {
		...
		case QMP_SESSION:
			logrus.Info("[RUNV] got new session")
			buf = append(buf, msg.(*QmpSession))
			if len(buf) == 1 {
				go qmpCommander(qc.qmp, conn, msg.(*QmpSession), res)
			}
		case QMP_FINISH:
			logrus.Infof("[RUNV] session finished, buffer size %d", len(buf))
			r := msg.(*QmpFinish)
			if r.success {
				logrus.Info("[RUNV] success ")
				if r.callback != nil {
					ctx.Hub <- r.callback
				}
			} else {
		...
		case QMP_EVENT:
			ev := msg.(*QmpEvent)
			logrus.Info("[RUNV] got QMP event ", ev.Type)
			if ev.Type == QMP_EVENT_SHUTDOWN {
				logrus.Info("[RUNV] got QMP shutdown event, quit...")
				handler = nil
		...
		case QMP_INTERNAL_ERROR:
			res <- msg
			handler = nil
			logrus.Info("[RUNV] QMP handler quit as received ", msg.(*QmpInternalError).cause)
	...
	initializing := func(msg QmpInteraction) {
		...
		case QMP_INIT:
			...
			conn = init.conn
			handler = loop
			logrus.Info("[RUNV] QMP initialzed, go into main QMP loop")
		...
		case QMP_SESSION:
			logrus.Info("[RUNV] got new session during initializing")
			buf = append(buf, msg.(*QmpSession))
		}
	}
	...
	for handler != nil {
		msg, ok := <-qc.qmp
		if !ok {
			logrus.Info("[RUNV] QMP channel closed, Quit qmp handler")
			break
		}
		handler(msg)
	}
